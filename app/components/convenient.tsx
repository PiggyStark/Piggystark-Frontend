...
export default function Convenient() {
    ...
    return (
        <section className=" w-full bg-[#1C1A1E] px-5 md:px-10 py-16 flex flex-col items-center justify-center gap-14 md:gap-30 relative overflow-hidden " >
            ...
            <h1 className="text-[#FBF8FF] font-medium md:text-[32px] text-center max-w-[550px] text-[25px] z-10  " >One convenient interface tailored
                For you</h1>
...

                {
                    convenientCards.map((convenientCard, index) => (
                        <div key={index} className={`flex items-center justify-center gap-5  md:gap-[42px] max-w-[825.91px] py-2 px-5   ${index + 1 === 2 ? "ml-auto" : "null"} `}   >
                            ...
                            <span className=" flex items-center justify-center bg-[#6D72C3] min-w-[70px] min-h-[70px] lg:w-[150px] lg:h-[150px] rounded-full text-[#F6F7FF] font-medium text-4xl lg:text-[52px] shadow-[inset_17px_17px_25.8px_0px_#33344340,inset_0px_4px_4px_0px_#00000040] " >
                                {index + 1}
                            </span>

                            <div className=" flex flex-col items-start justify-between gap-6 " >
                                <h2 className="text-[#FFFFFFCC] font-medium text-xl md:text-2xl "  > {convenientCard.heading} </h2>
                                <p className=" text-[#FFFFFFB2] font-medium text-lg md:text-2xl max-w-[599px] "  > {convenientCard.content} </p>
                            </div>
                        </div>
                    ))
                }
